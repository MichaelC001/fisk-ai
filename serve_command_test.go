...
var _ = Describe("fiskServeCommand", func() {
	...
	Describe("noEndpointError", func() {
		...
		It("Should name the file and show the blocks that fix it", func() {
			...
			Expect(err).To(MatchError(ContainSubstring("web:")))
			...
			Expect(err).To(MatchError(ContainSubstring(config.DefaultWebListen)))
			Expect(err).To(MatchError(ContainSubstring(config.DefaultWebBasePath)))
		})
	})
})

...
func decodeContent(g *WithT, stub tracetest.SpanStub, key attribute.Key) []map[string]any {
	g.THelper()

	v, ok := spanAttr(stub, key)
	g.Expect(ok).To(BeTrue(), "expected %s on span %q", key, stub.Name)

	var out []map[string]any
	g.Expect(json.Unmarshal([]byte(v.AsString()), &out)).To(Succeed(), "%s must be valid JSON: %s", key, v.AsString())
...
func spanNamed(g *WithT, exp *tracetest.InMemoryExporter, prefix string) tracetest.SpanStub {
	g.THelper()
	...
	g.Expect(found).To(HaveLen(1), "expected exactly one %q span, recorded: %v", prefix, names)

	return found[0]
}

// startupSpan returns the recorded startup span.
func startupSpan(g *WithT, exp *tracetest.InMemoryExporter) tracetest.SpanStub {
	g.THelper()

	return spanNamed(g, exp, "startup ")
}
...
func rootSpan(g *WithT, exp *tracetest.InMemoryExporter) tracetest.SpanStub {
	g.THelper()

	return spanNamed(g, exp, "invoke_agent ")
}
...
// TestTelemetry_StartupSpanRecordsToolInventory asserts a completed run exports one
// startup span named for the identity and carrying the resolved tool counts.
func TestTelemetry_StartupSpanRecordsToolInventory(t *testing.T) {
	g := NewWithT(t)
	...
	span := startupSpan(g, exp)
	g.Expect(span.Name).To(Equal("startup " + cfg.Identity))
	...
	g.Expect(v.AsInt64()).To(BeNumerically(">", 0))
	...
	for _, key := range []attribute.Key{
		...
		g.Expect(ok).To(BeTrue(), "expected %s to be set", key)
	}
}

// TestTelemetry_StartupSpanReportsTheMemoryBackendThatRan is the spec that makes reading
// the store worth doing at all.
//
...
func TestTelemetry_StartupSpanReportsTheMemoryBackendThatRan(t *testing.T) {
	g := NewWithT(t)
	...
	startup := startupSpan(g, exp)

	backend, ok := spanAttr(startup, telemetry.AttrMemoryBackend)
	g.Expect(ok).To(BeTrue())
	g.Expect(backend.AsString()).To(Equal("jetstream"))
	g.Expect(backend.AsString()).ToNot(Equal(string(cfg.MemoryBackend())))

	location, ok := spanAttr(startup, telemetry.AttrMemoryLocation)
	g.Expect(ok).To(BeTrue())
	g.Expect(location.AsString()).To(Equal("FISK_MEMORY"))
}

// TestTelemetry_StartupSpanOmitsAMemoryLocationThatIsNotSafeToName asserts a backend
// with nothing safe to name reports the backend alone.
//
...
func TestTelemetry_StartupSpanOmitsAMemoryLocationThatIsNotSafeToName(t *testing.T) {
	g := NewWithT(t)
	...
	backend, ok := spanAttr(startup, telemetry.AttrMemoryBackend)
	g.Expect(ok).To(BeTrue())
	g.Expect(backend.AsString()).To(Equal(string(memory.BackendFile)))

	_, ok = spanAttr(startup, telemetry.AttrMemoryLocation)
	g.Expect(ok).To(BeFalse())

	for _, kv := range startup.Attributes {
		g.Expect(kv.Value.String()).ToNot(ContainSubstring("/"))
	}
}

// TestTelemetry_MemoryIndexSpanNestsUnderStartup asserts the index load is its own span
// under startup, and that threading startup's context did not drag the run loop with it.
//
...
func TestTelemetry_MemoryIndexSpanNestsUnderStartup(t *testing.T) {
	g := NewWithT(t)
	...
	root := rootSpan(g, exp)
	startup := startupSpan(g, exp)
	index := spanNamed(g, exp, "memory_index")

	g.Expect(index.Parent.SpanID()).To(Equal(startup.SpanContext.SpanID()))
	g.Expect(startup.Parent.SpanID()).To(Equal(root.SpanContext.SpanID()))

	// The loop still hangs off the root, not off the span that ended at the handoff.
	chat := spanNamed(g, exp, "chat ")
	g.Expect(chat.Parent.SpanID()).To(Equal(root.SpanContext.SpanID()))
}

// TestTelemetry_MemoryToolSpanReportsTheBackendThatServedIt asserts a memory tool call
// says which store answered it.
//
...
func TestTelemetry_MemoryToolSpanReportsTheBackendThatServedIt(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(tool.Name).To(Equal("execute_tool memory_write"))
...
// TestTelemetry_NonMemoryToolSpanCarriesNoMemoryAttribution asserts the pair stays a
// filter for memory tools rather than becoming a label on every tool call.
//
...
func TestTelemetry_NonMemoryToolSpanCarriesNoMemoryAttribution(t *testing.T) {
	g := NewWithT(t)
	...
	tool := spanNamed(g, exp, "execute_tool ")
	g.Expect(tool.Name).To(Equal("execute_tool do"))
	...
	_, ok = spanAttr(startupSpan(g, exp), telemetry.AttrMemoryBackend)
	g.Expect(ok).To(BeTrue())
}

// TestTelemetry_MemoryAttributionFollowsARewrittenCall asserts the attribution describes
// the tool that ran, not the one the model asked for.
//
...
// call that did.
func TestTelemetry_MemoryAttributionFollowsARewrittenCall(t *testing.T) {
	g := NewWithT(t)
	...
	run := func(called string, rewrite agent.PreToolUseResult, input []byte) tracetest.SpanStub {
		...
		return spanNamed(g, exp, "execute_tool ")
	}
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(name.AsString()).To(Equal("do"))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(name.AsString()).To(Equal("memory_list"))
...
// TestTelemetry_StartupKeepsMemoryAttributionWithNoIndexAndNoToolCall asserts the pair
// stays on startup even once the tool spans carry it.
//
...
// existed at all.
func TestTelemetry_StartupKeepsMemoryAttributionWithNoIndexAndNoToolCall(t *testing.T) {
	g := NewWithT(t)
	...
	for _, span := range exp.GetSpans() {
		g.Expect(span.Name).ToNot(Equal("memory_index"))
		g.Expect(span.Name).ToNot(HavePrefix("execute_tool "))
	}
...
// TestTelemetry_StartupSpanEndsAtTheHandoff asserts the span closes when setup ends
// rather than when Run returns.
//
...
func TestTelemetry_StartupSpanEndsAtTheHandoff(t *testing.T) {
	...
	g.Expect(endedBeforeLoop).To(BeTrue(), "startup span had not ended by the time the run loop began")
}

// TestTelemetry_StartupSpanEndsOnAnEarlyReturn asserts a setup failure still exports
// the span, classified.
//
...
func TestTelemetry_StartupSpanEndsOnAnEarlyReturn(t *testing.T) {
	g := NewWithT(t)
	...
	_, err := agent.Run(context.Background(), agent.Options{
		...
		Provider:    agenttest.NewScriptedProvider(t, agenttest.TextResponse("done")),
		Telemetry:   tel,
		ToolWorkDir: filepath.Join(t.TempDir(), "does-not-exist"),
	}, agenttest.NewRecordingEvents(), agenttest.NewScriptedPrompter(t))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(v.AsString()).To(Equal(telemetry.ClassConfig.String()))
	...
	g.Expect(span.Status.Description).To(BeEmpty())
	for _, kv := range span.Attributes {
		g.Expect(kv.Value.String()).ToNot(ContainSubstring("does-not-exist"))
	}
}

// TestTelemetry_RootSpanIsTheWholeRun asserts a one-shot run's root is the agent
// invocation, that the startup span nests inside it, and that the run's outcome is on
// it.
//
...
func TestTelemetry_RootSpanIsTheWholeRun(t *testing.T) {
	g := NewWithT(t)
	...
	root := rootSpan(g, exp)
	g.Expect(root.Name).To(Equal("invoke_agent " + cfg.Identity))

	startup := startupSpan(g, exp)
	g.Expect(startup.Parent.SpanID()).To(Equal(root.SpanContext.SpanID()))
	g.Expect(startup.SpanContext.TraceID()).To(Equal(root.SpanContext.TraceID()))

	v, ok := spanAttr(root, telemetry.AttrRunTerminalReason)
	g.Expect(ok).To(BeTrue())
	g.Expect(v.AsString()).To(Equal("completed"))

	crashed, ok := spanAttr(root, telemetry.AttrRunCrashed)
	g.Expect(ok).To(BeTrue())
	g.Expect(crashed.AsBool()).To(BeFalse())

	// The run's own summary line points at the trace it produced, which is the only
	// correlator an un-checkpointed chat has.
	g.Expect(res.Stats.TraceID).To(Equal(root.SpanContext.TraceID().String()))
}

// TestTelemetry_RootSpanReportsSetupFailure asserts a run that never reached the loop
// still ends its root, classified.
//
...
func TestTelemetry_RootSpanReportsSetupFailure(t *testing.T) {
	g := NewWithT(t)
	...
	v, ok := spanAttr(root, telemetry.AttrRunTerminalReason)
	g.Expect(ok).To(BeTrue())
	g.Expect(v.AsString()).To(Equal("setup_failed"))

	class, ok := spanAttr(root, "error.type")
	g.Expect(ok).To(BeTrue())
	g.Expect(class.AsString()).To(Equal(telemetry.ClassConfig.String()))

	// res.Stats is nil on this path, so a root that read it without guarding would
	// crash the run it is meant to be reporting on.
	g.Expect(root.Status.Description).To(BeEmpty())
}

// TestTelemetry_InteractiveRunIsAWorkflowOverTurns asserts a chat run's shape: the
// root is a workflow, each turn is an agent invocation nested under it, and a one-shot
// run gets no turn span at all.
//
...
func TestTelemetry_InteractiveRunIsAWorkflowOverTurns(t *testing.T) {
	g := NewWithT(t)
	...
	root := spanNamed(g, exp, "invoke_workflow ")
	g.Expect(root.Name).To(Equal("invoke_workflow " + cfg.Identity))
	...
	for _, turn := range turns {
		g.Expect(turn.Parent.SpanID()).To(Equal(root.SpanContext.SpanID()))
	}
	...
	g.Expect(indexes).To(ConsistOf(int64(1), int64(2)))
}

// TestTelemetry_OneShotRunHasNoTurnSpan asserts the other half of that rule.
func TestTelemetry_OneShotRunHasNoTurnSpan(t *testing.T) {
	g := NewWithT(t)
	...
	root := rootSpan(g, exp)
	g.Expect(root.Parent.IsValid()).To(BeFalse())

	for _, span := range exp.GetSpans() {
		g.Expect(span.Name).ToNot(HavePrefix("invoke_workflow "))
	}
}

// TestTelemetry_ChatSpanRecordsTheCall asserts a model call gets a CLIENT span under
// the run, carrying the request shape, the reply's identifiers and its token tiers.
func TestTelemetry_ChatSpanRecordsTheCall(t *testing.T) {
	g := NewWithT(t)
	...
	_, err := agent.Run(context.Background(), agent.Options{
		...
		ConfigFile: "agent.yaml",
		Prompt:     []string{"go"},
		Provider:   agenttest.NewScriptedProvider(t, reply),
		Telemetry:  tel,
	}, agenttest.NewRecordingEvents(), agenttest.NewScriptedPrompter(t))
	g.Expect(err).ToNot(HaveOccurred())

	chat := spanNamed(g, exp, "chat ")
	g.Expect(chat.Name).To(Equal("chat " + cfg.LLM.Model))
	g.Expect(chat.SpanKind).To(Equal(trace.SpanKindClient))
	...
	for key, want := range map[attribute.Key]string{
		...
		g.Expect(ok).To(BeTrue(), "expected %s", key)
		g.Expect(v.AsString()).To(Equal(want))
	}
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(requested.AsString()).To(Equal(cfg.LLM.Model))
	g.Expect(requested.AsString()).ToNot(Equal("claude-sonnet-5-20260101"))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(input.AsInt64()).To(Equal(int64(152)))
...
// output cap is an error on the span.
//
...
func TestTelemetry_ChatSpanMarksATruncatedReplyFailed(t *testing.T) {
	g := NewWithT(t)
	...
	_, err := agent.Run(context.Background(), agent.Options{
		...
		ConfigFile: "agent.yaml",
		Prompt:     []string{"go"},
		Provider:   agenttest.NewScriptedProvider(t, truncated),
		Telemetry:  tel,
	}, agenttest.NewRecordingEvents(), agenttest.NewScriptedPrompter(t))
	g.Expect(err).To(HaveOccurred())

	chat := spanNamed(g, exp, "chat ")
	g.Expect(chat.Status.Code).To(Equal(codes.Error))

	class, ok := spanAttr(chat, "error.type")
	g.Expect(ok).To(BeTrue())
	g.Expect(class.AsString()).To(Equal(telemetry.ClassTruncated.String()))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(finish.AsStringSlice()).To(ConsistOf("max_tokens"))
}

// TestTelemetry_ToolSpanRecordsTheCall asserts a tool call gets a span carrying the
// tool, its kind, its outcome and its argument key names.
func TestTelemetry_ToolSpanRecordsTheCall(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(callID.AsString()).To(Equal("call_1"))

	out, ok := spanAttr(tool, telemetry.AttrToolOutcome)
	g.Expect(ok).To(BeTrue())
	g.Expect(out.AsString()).To(Equal(telemetry.ToolOutcomeExecuted))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(keys.AsStringSlice()).To(ConsistOf("subject"))
	for _, kv := range tool.Attributes {
		g.Expect(kv.Value.String()).ToNot(ContainSubstring("widgets"))
	}
}

// TestTelemetry_ToolSpanRecordsASuccessfulExitCode is the load-bearing assertion for
// fisk.tool.exit_code, and the one an implementer is most likely not to write.
//
...
func TestTelemetry_ToolSpanRecordsASuccessfulExitCode(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(ok).To(BeTrue(), "a command that exited zero must still report its status")
	g.Expect(code.AsInt64()).To(BeZero())
...
// TestTelemetry_ToolSpanOmitsExitCodeForAnInProcessTool asserts the attribute is absent,
// not zero, for a tool that ran no command.
//
...
func TestTelemetry_ToolSpanOmitsExitCodeForAnInProcessTool(t *testing.T) {
	g := NewWithT(t)
	...
	tool := spanNamed(g, exp, "execute_tool ")
	g.Expect(tool.Name).To(Equal("execute_tool memory_list"))

	// The kind is what tells an operator the absence is expected rather than a gap.
	kind, ok := spanAttr(tool, telemetry.AttrToolKind)
	g.Expect(ok).To(BeTrue())
	g.Expect(kind.AsString()).To(Equal("builtin"))

	_, ok = spanAttr(tool, telemetry.AttrToolExitCode)
	g.Expect(ok).To(BeFalse())
}

// TestTelemetry_UnknownToolIsNotSpanNameCardinality asserts a tool name the model
// invented cannot reach a span name.
//
...
func TestTelemetry_UnknownToolIsNotSpanNameCardinality(t *testing.T) {
	g := NewWithT(t)
	...
	tool := spanNamed(g, exp, "execute_tool ")
	g.Expect(tool.Name).To(Equal("execute_tool unknown_tool"))
	g.Expect(tool.Name).ToNot(ContainSubstring(invented))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(requested.AsString()).To(Equal(invented))

	out, ok := spanAttr(tool, telemetry.AttrToolOutcome)
	g.Expect(ok).To(BeTrue())
	g.Expect(out.AsString()).To(Equal(telemetry.ToolOutcomeUnknownTool))
}

// TestTelemetry_ToolSpanRecordsAPolicyDenial asserts a call a hook refused is still a
// span, distinguishable from one that ran and failed.
//
...
func TestTelemetry_ToolSpanRecordsAPolicyDenial(t *testing.T) {
	g := NewWithT(t)
	...
	out, ok := spanAttr(tool, telemetry.AttrToolOutcome)
	g.Expect(ok).To(BeTrue())
	g.Expect(out.AsString()).To(Equal(telemetry.ToolOutcomePolicyDenied))
...
// span that ends, classified, and carrying no stack.
//
...
func TestTelemetry_RootSpanEndsOnACrash(t *testing.T) {
	g := NewWithT(t)
	...
	crashed, ok := spanAttr(root, telemetry.AttrRunCrashed)
	g.Expect(ok).To(BeTrue())
	g.Expect(crashed.AsBool()).To(BeTrue())

	class, ok := spanAttr(root, "error.type")
	g.Expect(ok).To(BeTrue())
	g.Expect(class.AsString()).To(Equal(telemetry.ClassPanic.String()))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(reason.AsString()).To(Equal("error"))

	g.Expect(root.Events).To(BeEmpty())
	_, ok = spanAttr(root, "exception.stacktrace")
	g.Expect(ok).To(BeFalse())
	g.Expect(root.Status.Description).To(BeEmpty())
}

// TestTelemetry_NilProviderRunsClean asserts the default path, telemetry unwired,
// runs to completion. Every call site calls the facade unconditionally, so a nil
// Provider that was not safe would panic in the run every operator has.
func TestTelemetry_NilProviderRunsClean(t *testing.T) {
	g := NewWithT(t)
...
// TestTelemetry_HTTPAttemptsAreRecordedPerRetry asserts a rate-limited model call
// reports each HTTP attempt as its own event and the retry as a resend count.
//
...
func TestTelemetry_HTTPAttemptsAreRecordedPerRetry(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(resends.AsInt64()).To(Equal(int64(1)))
	...
	g.Expect(eventKeys(attempts[0])).To(ConsistOf(
...
// TestTelemetry_HTTPTransportFailureIsRecordedWithoutAStatus asserts an attempt that
// never got a response is reported as an error event and does not crash the run.
//
...
func TestTelemetry_HTTPTransportFailureIsRecordedWithoutAStatus(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(failures).ToNot(BeEmpty())
	g.Expect(eventsNamed(chat, telemetry.EventLLMHTTPResponse)).To(BeEmpty())
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(class.AsString()).To(Equal(telemetry.ClassProvider.String()))

	for _, e := range failures {
		g.Expect(eventKeys(e)).To(ConsistOf(
			string(telemetry.AttrLLMHTTPAttempt),
			string(telemetry.AttrLLMHTTPDurationMS),
			"error.type",
		))
...
// TestTelemetry_RetrievalNestsUnderTheToolCall asserts a knowledge search opened by the
// model produces a retrieval span underneath the execute_tool span that dispatched it.
//
...
func TestTelemetry_RetrievalNestsUnderTheToolCall(t *testing.T) {
	g := NewWithT(t)
	...
	tool := spanNamed(g, exp, "execute_tool ")
	g.Expect(tool.Name).To(Equal("execute_tool knowledge_search"))

	retrieval := spanNamed(g, exp, "retrieval")
	g.Expect(retrieval.Parent.SpanID()).To(Equal(tool.SpanContext.SpanID()))
	g.Expect(retrieval.SpanContext.TraceID()).To(Equal(tool.SpanContext.TraceID()))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(tier.AsString()).To(Equal(telemetry.TierLexical))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(toolType.AsString()).To(Equal(telemetry.ToolTypeDatastore))
}

// TestTelemetry_ContentIsAbsentWhenCaptureIsOff is the spec that has to hold whatever
// else changes: a run with telemetry on and capture unset exports structure and timing
...
func TestTelemetry_ContentIsAbsentWhenCaptureIsOff(t *testing.T) {
	g := NewWithT(t)
	...
	spans := exp.GetSpans()
	g.Expect(spans).ToNot(BeEmpty())

	for _, span := range spans {
		for _, key := range contentKeys {
			_, ok := spanAttr(span, key)
			g.Expect(ok).To(BeFalse(), "span %q carries %s with capture off", span.Name, key)
		}
		_, ok := spanAttr(span, telemetry.AttrContentFromIndex)
		g.Expect(ok).To(BeFalse(), "span %q carries a content marker with capture off", span.Name)
	}
}

// TestTelemetry_ChatSpanCarriesTheConversation asserts the model call carries what was
// sent and what came back, each as a document that parses.
func TestTelemetry_ChatSpanCarriesTheConversation(t *testing.T) {
	g := NewWithT(t)
	...
	in := decodeContent(g, chat, "gen_ai.input.messages")
	g.Expect(in).To(HaveLen(1))
	g.Expect(in[0]["role"]).To(Equal("user"))

	out := decodeContent(g, chat, "gen_ai.output.messages")
	g.Expect(out).To(HaveLen(1))
	g.Expect(out[0]["role"]).To(Equal("assistant"))
	g.Expect(out[0]["finish_reason"]).ToNot(BeEmpty())
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(idx.AsInt64()).To(BeZero())
}

// TestTelemetry_InputIsTheDeltaAndTheIndexesChain is the spec the whole delta design
// rests on.
//
...
func TestTelemetry_InputIsTheDeltaAndTheIndexesChain(t *testing.T) {
	g := NewWithT(t)
	...
	// exported.
	second := decodeContent(g, chats[1], "gen_ai.input.messages")
	g.Expect(second).To(HaveLen(2))
	g.Expect(second[0]["role"]).To(Equal("assistant"))
	g.Expect(second[1]["role"]).To(Equal("tool"))
	...
	for _, chat := range chats {
		...
		exported := decodeContent(g, chat, "gen_ai.input.messages")
		g.Expect(int(idx.AsInt64())+len(exported)).To(Equal(int(sent.AsInt64())),
			"from_index plus the exported messages must account for every message sent")
	}
}

// TestTelemetry_FullMessagesCarriesTheWholeConversation asserts the opt-out does what
// its name says, which the delta spec above cannot see.
func TestTelemetry_FullMessagesCarriesTheWholeConversation(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(decodeContent(g, last, "gen_ai.input.messages")).To(HaveLen(3))

	idx, ok := spanAttr(last, telemetry.AttrContentFromIndex)
	g.Expect(ok).To(BeTrue())
	g.Expect(idx.AsInt64()).To(BeZero(), "the full conversation always starts at the beginning")
}

// TestTelemetry_SystemPromptIsOnStartupOnce asserts the run constant is exported once,
// on the span that ends early, and not repeated on every model call.
//
...
// iteration.
func TestTelemetry_SystemPromptIsOnStartupOnce(t *testing.T) {
	g := NewWithT(t)
	...
	for _, span := range exp.GetSpans() {
		...
		carriers++
		g.Expect(span.Name).To(HavePrefix("startup "))
	}
	g.Expect(carriers).To(Equal(1), "the system prompt is a run constant and belongs on exactly one span")

	instructions := decodeContent(g, startupSpan(g, exp), "gen_ai.system_instructions")
	g.Expect(instructions).ToNot(BeEmpty())
	g.Expect(instructions[0]["type"]).To(Equal("text"))
}

// TestTelemetry_SystemPromptIsCapturedAfterEveryAppend is the spec that catches the
// call-point trap, and nothing else does.
//
...
func TestTelemetry_SystemPromptIsCapturedAfterEveryAppend(t *testing.T) {
	g := NewWithT(t)
	...
	v, ok := spanAttr(startupSpan(g, exp), "gen_ai.system_instructions")
	g.Expect(ok).To(BeTrue())
	...
	g.Expect(v.AsString()).To(ContainSubstring("deploy-notes"))
}

// TestTelemetry_ToolSpanCarriesArgumentsAndResult asserts a tool call carries what it
// was given and what it returned.
func TestTelemetry_ToolSpanCarriesArgumentsAndResult(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(json.Unmarshal([]byte(args.AsString()), &obj)).To(Succeed())
	g.Expect(obj).To(HaveKeyWithValue("subject", "widgets"))

	res, ok := spanAttr(tool, "gen_ai.tool.call.result")
	g.Expect(ok).To(BeTrue())
	g.Expect(json.Valid([]byte(res.AsString()))).To(BeTrue())
}

// TestTelemetry_DeniedToolCallCarriesWhatTheModelWasTold asserts a call that never ran
// still records the answer the model acted on.
//
...
func TestTelemetry_DeniedToolCallCarriesWhatTheModelWasTold(t *testing.T) {
	g := NewWithT(t)
	...
	res, ok := spanAttr(tool, "gen_ai.tool.call.result")
	g.Expect(ok).To(BeTrue(), "the model was told why, and that is what it acted on")
	g.Expect(res.AsString()).To(ContainSubstring("not in this environment"))

	_, ok = spanAttr(tool, "gen_ai.tool.call.arguments")
	g.Expect(ok).To(BeTrue(), "a denied call still exports its arguments")
}

// TestTelemetry_OversizedContentStaysValidAndSaysSo asserts the truncation markers
// reach the span, since the genai specs prove the documents and this proves they are
// reported.
func TestTelemetry_OversizedContentStaysValidAndSaysSo(t *testing.T) {
	g := NewWithT(t)
	...
	decodeContent(g, chat, "gen_ai.input.messages")

	cut, ok := spanAttr(chat, telemetry.AttrContentTruncated)
	g.Expect(ok).To(BeTrue())
	g.Expect(cut.AsStringSlice()).To(ContainElement("gen_ai.input.messages"))
}

// TestTelemetry_MemoryIndexSpanCountsTheEntries asserts the index load reports how much
// work it was, not only how long it took.
//
...
func TestTelemetry_MemoryIndexSpanCountsTheEntries(t *testing.T) {
	g := NewWithT(t)
	...
	for _, key := range []string{"deploy-notes", "api-conventions", "oncall"} {
		g.Expect(store.Write(context.Background(), key, "a description", "body", false)).To(Succeed())
	}
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(entries.AsInt64()).To(BeEquivalentTo(3))
}

// TestTelemetry_MemoryIndexSpanOmitsTheCountWhenTheLoadFailed is the spec an
// implementer skips, and the only one that fails against the obvious implementation.
//
...
// different places.
func TestTelemetry_MemoryIndexSpanOmitsTheCountWhenTheLoadFailed(t *testing.T) {
	g := NewWithT(t)
	...
	g.Expect(ok).To(BeFalse(), "a failed load has no count, and zero would mean an empty store")

	class, ok := spanAttr(index, "error.type")
	g.Expect(ok).To(BeTrue())
	g.Expect(class.AsString()).To(Equal(telemetry.ClassStore.String()))
	...
	g.Expect(ok).To(BeTrue())
	g.Expect(backend.AsString()).ToNot(BeEmpty())

	// The store's own message names a bucket, and only the class may leave the process.
	g.Expect(index.Status.Description).To(BeEmpty())
	for _, kv := range index.Attributes {
		g.Expect(kv.Value.String()).ToNot(ContainSubstring("FISK_MEMORY"))
	}
}

// TestTelemetry_MemoryIndexClassifiesCancellationAsCancellation asserts an interrupted
// setup is not reported as a broken memory store.
//
...
func TestTelemetry_MemoryIndexClassifiesCancellationAsCancellation(t *testing.T) {
	g := NewWithT(t)

...
// Package wire is the v1 message vocabulary of the fisk-ai agent-to-agent (A2A)
// protocol, the JSON schemas that define it, and the Validator that checks a body
// against them. It imports the standard library, jsonschema and ksuid and nothing
// else, so a program that only parses or emits the protocol pays for nothing more.
//
...
// captured message is fully self describing outside of any transport. A reply set is
// correlated by the Header.Request id and ordered by Header.Sequence. The client, the
// server and the transport bindings that carry these messages are in internal/a2a.
//
...
// family. The matching JSON schemas are embedded from the schemas directory of this package
// and published under https://choria.io/schemas/io.choria.fisk-ai.v1, one per id, each
...
//
//	res, err := wire.DecodeTerminal(data)
//	var failed *wire.ErrorMessage

...
mcp_clients:
  - # Identifies the server in errors and prefixes every tool imported from
    ...
    name: filesystem
    # A shorter prefix used in place of "name" on imported tool names, under
    # the same character rules. Defaults to "name".
    alias: fs
    # The program to start. Setting it selects the stdio transport; "url" and
    # "headers" may not be set alongside it.
    command: npx
    # Arguments to "command", one per entry. Both are literal text: a
    # "${VAR}" written in either reaches the program as it stands.
    args:
      - -y
    ...
    # rejected.
    timeout: 30s
    # Which of the server's tools to import. Regular expressions matched
    # against the server's own tool name, before the alias prefix. Include
    # runs first and exclude drops from what it kept. A "tags" filter is
    # rejected on either: MCP has no tag vocabulary, so a tag filter could
    # never be honored.
    include:
      tools:
        - ^read_
...
[MCP client](../agents/mcp-client/) guide covers this end to end.

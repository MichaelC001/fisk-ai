...
func validateRunTarget(cfg *config.Config, remote bool) error {
	if !remote {
		...
		if !cfg.SuppliesTools() {
			return fmt.Errorf("no tools available: this agent wraps no application (application_path unset) and enables no built-in, remote or mcp tools; set application_path, list a tool under harness.tools, or enable harness.knowledge, harness.memory, human_in_the_loop, remote_tools or mcp_clients in %q", configFile)
		}
